    ...
    *,
    ...
    if trigger_phase_s is not None:
        ...
        for window in windows:
            ...
            if end <= 0:
                start += waveform.achieved_period_s
                end += waveform.achieved_period_s
            shifted.append(
                MeasurementWindow(
                    name=window.name,
                    role=window.role,
...
def measure_frame(
    ...
    if not np.all(np.isfinite(time_axis)) or not np.all(np.isfinite(voltage)):
        raise ValueError("frame contains non-finite time or ch1 values")
    ...
    for window in plan.windows:
        ...
        if selected.size == 0:
            raise ValueError(
                f"frame contains no points for measurement window {window.name!r} "
                f"({window.start_s:g} to {window.end_s:g} s)"
            )
        values_by_role.setdefault(window.role, []).append(selected)

    means: dict[str, float] = {}
    counts: dict[str, int] = {}
    for role, arrays in values_by_role.items():
